...
**BRAHMA** Yes, indeed! The goal of Parallelism is Performance while preserving the functionality of the system, whereas the goal of concurrency is Responsiveness.  While both structure the program for achieving different goals and use threads for their implementations, it is important to determine whether these threads interact with each other or run independently of each other interacting with multiple external clients.  
