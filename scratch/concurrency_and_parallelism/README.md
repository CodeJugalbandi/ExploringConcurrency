...
**BRAHMA** Yes, indeed! The goal of Parallelism is Performance while preserving the functionality of the system, whereas the goal of Concurrency is Responsiveness.  While both structure the program for achieving different goals and use threads for their implementations, it is important to determine whether these threads interact with each other to return a single result to the caller  or run independently of each other interacting with many external clients at the same time.  
