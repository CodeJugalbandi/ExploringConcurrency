...
public class RealTimeNationalStockServiceObservable {
	...
	public static void main(String[] args) throws URISyntaxException, IOException {
		...
		Observable<Double> prices = RealTimeNationalStockServiceObservable.forAllStocks()
		// Observable<Double> prices = RealTimeNationalStockServiceObservable.forStock("AMZN")
		    .map(data -> new JSONObject(data))
			.filter(message -> message.has("ticker"))
			.map(ticker -> ticker.getDouble("price"));
